...
현재 `StockPriceSubscriptionService`는 DB에 저장된 종목 목록을 한 번에 조회한 뒤, 여러 종목을 묶어서 KIS 구독 요청을 보낸다.

`StockOrderBookSubscriptionService`는 호가 화면 요구사항에 맞춰 단일 종목 구독으로 변경했다.
