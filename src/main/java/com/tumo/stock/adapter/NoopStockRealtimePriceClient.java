...
/**
 * 실제 실시간 시세 provider 구현이 붙기 전까지 구독 요청을 로그로만 처리하는 기본 client adapter.
 */
@Slf4j
@Component
...
public class NoopStockRealtimePriceClient implements StockRealtimePriceClient {
    ...
    /**
     * 실시간 가격 이벤트 구독을 외부 provider에 전달하지 않고 로그로만 남긴다.
     *
    ...
    /**
     * 실시간 가격 이벤트 구독 해제를 외부 provider에 전달하지 않고 로그로만 남긴다.
     *
